...
export class FaqDetectionAssistantApp extends App implements IPostMessageSent {
    ...
    private async checkDynamicFaqs(read: IRead, query: string): Promise<{ answer: string; confidence: number } | null> {
        ...
        for (const faq of dynamicFaqs as any[]) {
            ...
            if (score > bestScore && score >= 0.9) { // BM threshold: 90%
                bestScore = score;
    ...
    private async refineWithLLM(http: IHttp, read: IRead, query: string, persistence: IPersistence): Promise<{ response: string; confidence: number; intent: string }> {
        ...
        const faqContext = Object.entries(this.faqDatabase).map(([q, a]) => `Q: ${q}\nA: ${a}`).join('\n');
        const dynamicFaqs = await read.getPersistenceReader().readByAssociations(
        ...
        // Advanced CoT Prompt
        const prompt = `
You are a Rocket.Chat FAQ expert. Answer "${query}" step-by-step:  
1. Detect intent: "urgent" (e.g., "now"), "technical" (e.g., "how"), "casual" (e.g., "what’s").  
2. Match FAQs:  
${fullFaqContext}  
3. If unsure, say "I’m not sure." Respond < 50 words, tone matching intent.  
4. Provide confidence (0.0-1.0), cautious if uncertain.  
...
**Output (JSON)**: {"response": "...", "confidence": float, "intent": "..."}
    ...
    private async sendMessage(modify: IModify, room: IRoom, text: string): Promise<void> {
        try {
            const msg = modify.getCreator().startMessage().setRoom(room).setText(`🤖 **FAQ Bot:** ${text}`);
    ...
    private async sendAck(modify: IModify, room: IRoom): Promise<void> {
        await this.sendMessage(modify, room, 'Digging into that for you...');
    }
...
- \`@faq-bot reject ${originalMessageId}\` - Discard  
    ...
    private async handleModeratorCommand(
        ...
        if (command === 'clear-persistence') {
            ...
            await this.sendMessage(modify, room, 'Persistence layer cleared—starting fresh!');
            return true;
        }

        if (!originalMessageId || parts.length < 3) {
            await this.sendMessage(modify, room, 'Invalid command. Use: `@faq-bot <approve|edit|reject> <id> ["new text"]`');
            return true;
        }
        ...
        if (!pending) {
            await this.sendMessage(modify, room, `No pending response found for ID '${originalMessageId}'.`);
            return true;
        }

        const originalRoom = await read.getRoomReader().getById((pending as any).originalRoomId);
        if (!originalRoom) {
            await this.sendMessage(modify, room, `Error: Original room ${(pending as any).originalRoomId} not found.`);
            await persistence.remove((pending as any)._id || originalMessageId);
            return true;
        }

        if (command === 'approve') {
            await this.sendMessage(modify, originalRoom, (pending as any).response);
            await this.storeDynamicFaq(persistence, (pending as any).originalText, (pending as any).response, 'approved');
            await persistence.remove((pending as any)._id || originalMessageId);
            await this.sendMessage(modify, room, `Approved and sent to ${(pending as any).originalRoomId}. Added to FAQs.`);
            return true;
        } else if (command === 'edit' && parts.length > 3) {
            const newTextMatch = text.match(/"(.+)"/);
            const newText = newTextMatch ? newTextMatch[1] : null;
            if (!newText) {
                await this.sendMessage(modify, room, `Error: Use format: @faq-bot edit ${originalMessageId} "new text"`);
                return true;
            }
            await this.sendMessage(modify, originalRoom, newText);
            await this.storeDynamicFaq(persistence, (pending as any).originalText, newText, 'approved');
            await persistence.remove((pending as any)._id || originalMessageId);
            await this.sendMessage(modify, room, `Edited and sent to ${(pending as any).originalRoomId}. Added to FAQs.`);
            return true;
        } else if (command === 'reject') {
            await persistence.remove((pending as any)._id || originalMessageId);
            await this.sendMessage(modify, room, `Rejected response for ID ${originalMessageId}.`);
            return true;
        }

        await this.sendMessage(modify, room, `Unrecognized command '${command}'. Use: approve, edit, reject.`);
        return true;
    }
    ...
    private isFaqQuery(text: string, senderId?: string): boolean {
		...
		const faqKeywords = [
			'password', 'reset', 'refund', 'policy', 'support', 'contact', 'billing', 'username',
			'payment', 'account', 'delete', 'recover', 'authentication', 'download', 'app',
			'order', 'track', 'subscription', 'trial', 'bug', 'issue', 'customize', 'theme',
			'color', 'profile', 'settings', 'channel', 'message', 'notification', 'integration',
			'export', 'invite', 'team', 'emoji', 'layout', 'rocket.chat', 'rocket', 'chat', // Added Rocket.Chat terms
			'help', 'howto', 'guide', 'info', 'feature', 'plan', 'upgrade', 'downgrade', // Broader FAQ terms
			'enable', 'disable', 'change', 'update', 'create', 'join', 'leave', 'mute' // Action-oriented
		];
		...
		const questionStarters = [
			'how', 'what', 'where', 'when', 'why', 'is', 'are', 'can', 'do', 'does',
			'will', 'should', 'could', 'would', 'who', 'which' // More natural question words
		];
		const isQueryLike =
			lowerText.includes('?') || // Direct question
			questionStarters.some((starter) => lowerText.startsWith(starter)) || // Starts with question word
			(words.length >= 2 && words.some((word) => word.length >= 3)); // Looser: 2+ words, one 3+ chars
    ...
    private async clearPersistence(persistence: IPersistence, read: IRead): Promise<void> {
        try {
            ...
            for (const assoc of associations) {
                ...
                if (records.length === 0) {
                    console.log(`ℹ️ No records found for association: ${(assoc as any).id}`);
                    continue;
                }
    ...
    public async executePostMessageSent(
        ...
        try {
            ...
            console.log(`ℹ️ Processing message from room ID: ${oroomId}`);
            if (oroomId === 'GENERAL') {
                const generalRoom = await read.getRoomReader().getByName('general');
                if (generalRoom) {
                    oroomId = generalRoom.id;
                    console.log(`ℹ️ Corrected room ID to ${oroomId}`);
                } else {
            ...
            if (await this.handleModeratorCommand(modify, persistence, read, text, message.room)) {
                return;
            }
            ...
            if (dynamicMatch) {
                await this.sendMessage(modify, message.room, dynamicMatch.answer);
            ...
            if (llmResult.confidence >= threshold) {
                ...
                await this.sendMessage(modify, message.room, `${prefix} ${llmResult.response}`);
            } else {
                await this.sendAck(modify, message.room);
                ...
                await this.notifyModerators(modify, read, originalMessageId, message.text!, llmResult.response, oroomId);
            }
            this.lastResponseTime[oroomId] = currentTime;
        } catch (error) {
